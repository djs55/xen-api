...
module Vdi = struct
	...
	type t = {
		blkback: blkback option;                (** Some _ when attached; None otherwise *)
	...
	let to_string_list x =
		let title = Printf.sprintf "%s (device=%s)" (Vdi_automaton.string_of_state (superstate x)) (Opt.default "None" (Opt.map (fun x -> "Some " ^ (string_of_blkback x)) x.blkback)) in
...
module Wrapper = functor(Impl: Server_impl) -> struct
	...
	module VDI = struct
				...
				let result, vdi_t =
					try
						...
						| Vdi_automaton.Attach ro_rw ->
							...
							| Success (Vdi x) ->
								result, { vdi_t with Vdi.blkback = Some x }
							| Success Unit
							| Failure _ ->
								result, vdi_t
							| Success (State _) 
				...
				try
					...
					let result = match result, this_op with
						| Success _, Vdi_automaton.Attach _ ->
							Success (Vdi (Opt.unbox vdi_t.Vdi.blkback))
						| x, _ -> x in
		...
		let snapshot context ~task ~sr ~vdi ~params =
			info "VDI.snapshot task:%s sr:%s vdi:%s params:%s" task sr vdi (String.concat ";" (List.map (fun (k, v) -> k ^ ":" ^ v) params));
			with_vdi sr vdi
				(fun () ->
					Impl.VDI.snapshot context ~task ~sr ~vdi ~params
				)
