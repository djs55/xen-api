...
let create  ~__context ~device ~network ~vM
	...
	let mAC, mAC_autogenerated = match mAC with
		| "random" -> Device.Vif.random_local_mac (), true
		| "" ->
			begin match vm_mac_seed with
				| Some seed ->
					Device.Vif.hashchain_local_mac (int_of_string device) seed, true
				| None ->
					warn "No MAC supplied and no mac_seed on VM";
					mAC, false
			end
		| _ -> mAC, false in
